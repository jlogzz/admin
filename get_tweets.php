<?php
	...
	class ctwitter_stream
	{
		...
		{
			if($_data['id_str']!=NULL){
				$event=R::load('events',6);
				$tweet=R::dispense('tweets');
				$tweet->tweetid=$_data['id_str'];
				$tweet->datestr=strtotime($_data['created_at']);
				$tweet->from=$_data['user']['screen_name'];
				$tweet->fromimg=$_data['user']['profile_image_url'];
				foreach($_data['entities']['user_mentions'] as $mention){
					$mentions=$mentions."|".$mention['screen_name'];
				}
				$tweet->mentions=$mentions;
				$tweet->message=$_data['text'];
				foreach($_data['entities']['hashtags'] as $hashtag){
					$hashtags=$hashtags."|".$hashtag['text'];
				}
				$tweet->hashtags=$hashtags;
				foreach($_data['entities']['urls'] as $url){
